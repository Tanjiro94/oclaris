...
describe('GET /api/da (integration)', () => {
    ...
    test('returns 200 and a valid DA list when authenticated', async () => {
        ...
        expect(first).toHaveProperty('styles');
        expect(Array.isArray(first.styles)).toBe(true);
        expect(first).toHaveProperty('pictures');
        expect(Array.isArray(first.pictures)).toBe(true);
